...
export default {
  ...
  fix(editor, isAuto) {
    ...
    if (textlint.stdout) {
      ...
      const re = /Fixed (.*) problem/g;
      const msg = `Textlint Auto Fixer: ${textlint.stdout.match(re)}`;
    ...
    if (!textlint.stdout && !textlint.stderr && textlint.status === 0 && isAuto === false) {
      atom.notifications.addInfo('Textlint Auto Fixer: Nothing to fix', { dismissable: true });
    }
